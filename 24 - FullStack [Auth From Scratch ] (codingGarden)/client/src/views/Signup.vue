<template>
  <section>
    <h1>Signup</h1>
    <div v-if="signingUp">
      <img src="../assets/pacman-loading.svg" alt="" />
    </div>
...
export default {
  data: () => ({
    // here is all the properties you want
    signingUp: false,
    errorMessage: '',
    user: {
      username: '',
  ...
  watch: {
    // it's like subscribiton in angular [RXJS]
    user: {
      handler() {
        // if it really changed execute this
        this.errorMessage = '';
      },
      deep: true, // deep in the user object
    },
  },
  methods: {
    // here you define all the methods you need to use
    signup() {
      this.errorMessage = '';
      if (this.validUser()) {
        // send data to server
